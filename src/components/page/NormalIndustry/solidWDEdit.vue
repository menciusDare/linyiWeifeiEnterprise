<template>
                ...
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="申报类型:" :rules="ruleListA.applyType">
                               <el-select v-model="labelForm.quarter" disabled @change="yearOrquarter(labelForm.quarter)"  clearable>
                    <el-option
                ...
                <el-row >
                    <el-col :span="24" style='text-align: center;margin-top:20px;'>
                        <el-button type="primary" v-if="this.controlSeeNot==1 " @click="submitFormNew">提交</el-button>
                           <!-- <el-button type="primary" v-if="this.controlSeeNot==1 && newSubmitBtn " @click="submitFormAddUse">提交New</el-button> -->
    ...
    <el-select v-model="item.fA" placeholder="请选择" @change="selectNodeAA(item.fA)" >
                ...
                >
                    <input type="text" style="width:100%;border:none; height:44px;text-align:center" :disabled="true" v-model=" item.fB"/>  
                ...
                <td
                  ...
                  :title="item.fF"
                >
                 <input type="text" style="width:100%;border:none; height:44px;text-align:center"  v-model=" item.fF"/>               
                </td>   
              </tr>     
            </tbody>
          </table>
    ...
    v-model="item.entId"
     @change="changeCompany(item.entId,index)"
    filterable
    remote
    reserve-keyword
    placeholder="请输入关键词"
...
obj.name=item.name
  ...
  solidWasteVoluntarilyHandleList:this.solidWasteVoluntarilyHandleList,
      ...
      changeCompany(idA,index){
      this.optionsAA.forEach(item=>{
        if(item.id==idA){
 this.solidWasteRemoveList[index].entAddressd=item.address;
this.solidWasteRemoveList[index].contacts= item.contacts;
this.solidWasteRemoveList[index].contactsPhone=item.contactsPhone
        }
      })
...
//  let lastT =this.tableDataBottom.length-1;
// this.tableDataBottom.splice(lastT,1)
this.tableDataBottom.push(obj)
...
obj.type=item.dicName
   ...
   this.AllTableData.unshift(res.enterpriseSolidWasteDeclareDetailsList[i])     
...
this.solidWasteRemoveList=this.finalTableData
     ...
     let url =  `/api/regulatory/type/listByTypeId/${idN}`
      ...
      this.showSaveBtn=false,
                ...
                this.oldSubmitBtn=false,
                this.newSubmitBtn=true, 
      this.dialogVisible = true;
      this.cancleSee()
    },
     rowEdit(rowId, row) {
         this.btmRowId=rowId
         this.whichItem=row.idxx
     ...
     this.firstTable[0].fB=''
      this.firstTable[0].fC=''
        this.firstTable[0].fD =''
         ...
         this.firstTable[0].fF  =''
        //第二个表赋值
        this.solidWasteStorageList=[{beforeAmount:"",declareDetailId: "",deleteFlag: "",id: "",keepingMode: '',storagePositionId: '',storagePositionName: "",
totalAmount: "",transferName: "",updatedBy: "",updatedTime: ""}]

     this.solidWasteVoluntarilyHandleList=[{declareDetailId: "",deleteFlag: "",handleAmount: "",id: "",manageMode: "",
manageModeDetails: "",name: "",updatedBy: "",updatedTime: ""}]
 
       this.solidWasteRemoveList=[{contacts: "",contactsPhone: "",declareDetailId: "",deleteFlag: "",entAddressd: "",
entId: "",handleAmount: "",id: "",manageMode: "",manageModeDetails: "",transportEntId: "",updatedBy: "",updatedTime: "",utilizeAmount: "",utilizeHandleAmount: "",}]
...
param.enterpriseSolidWasteDeclareDetailsList=this.AllTableData
...
//  this.originalData.push(this.enterpriseSolidWasteDeclareDetailsList[0])
// param.enterpriseSolidWasteDeclareDetailsList=this.originalData
