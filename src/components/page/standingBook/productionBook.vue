                ...
                <tr v-for="(item, i) in tableDatas2" :key="i">
                  ...
                  <td style="text-align: center; width: 15%">
                    ...
                    <a
                      ...
                      >打印</a>
                    <a
                      v-if="item.operationType == 1 && item.pushFlag == 0"
                      href="javascript:;"
                      class="text-primary text-underline"
                      @click="confirmPush(item)"
                    >推送</a>
                  </td>
                </tr>
              </tbody>
  ...
  methods: {
    confirmPush(params) {
      ...
      this.$msgbox({
        title: '提示',
        message: h('p', null, [
          h('span', null, '此操作将推送贮存台账到省平台, 是否继续? ')
        ]),
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            this.$axios.get("/api/provincial/accountPush/addReceipt?entId="+params.entId+"&storageBookId="+params.id).then((res) => {
              done();
              if (res.code == 0) {
                this.$message({
                  type: "success",
